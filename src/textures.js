...
/** A banner carrying an organisation's crest above its name. */
export function logoFlagTexture(renderer, { org, title, color = '#8f83f7', width = 512, height = 320 }) {
  const key = `logoflag|${org}|${title}|${color}|${width}x${height}`
  if (cache.has(key)) return cache.get(key)
  ...
  ctx.textAlign = 'center'
  ctx.textBaseline = 'middle'
  const f = fitLines(ctx, title, width * 0.76, Math.round(height * 0.13), 800, 2)
  ctx.font = `800 ${f.size}px ${FONT_STACK}`
  ctx.fillStyle = '#eef4ff'
  let y = height * 0.76 - ((f.lines.length - 1) * f.size * 1.1) / 2
  for (const line of f.lines) {
    ctx.fillText(line, width * 0.53, y)
    y += f.size * 1.1
  }
